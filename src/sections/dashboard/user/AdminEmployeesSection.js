...
const AdminEmployeesSection = ({ setShowEmp }) => {
  const { employees, error, message, actionT } = useSelector(
    ...
    <>
      <Box p={2} sx={{ height: "100%" }}>
        <Box className="d-flex justify-content-between align-items-center">
          <Typography variant="h2" className="text-dark fw-semibold">
            Employees
          </Typography>
          <Button className="btn--dark" onClick={() => setShowEmp(false)}>
            Add New Employee
          </Button>
        </Box>
