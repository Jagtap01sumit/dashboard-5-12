...
export const getEmployeeDetail = (id) => async (dispatch) => {
  try {
    dispatch({ type: actionTypes.CLEAR_EMPLOYEE_STATE });
    const response = await fetch("/api/admin/get-employee-details", {
      method: "POST",
    ...
    if (success) {
      ...
      dispatch({
        type: actionTypes.GET_EMPLOYEE_DETAILS_FAILURE,
        payload: message,
      });
    }
  } catch (error) {
...
export const removeEmployee = (id) => async (dispatch) => {
  try {
    const response = await fetch("/api/admin/remove-employee", {
      method: "POST",
      body: JSON.stringify({ id }),
    });
